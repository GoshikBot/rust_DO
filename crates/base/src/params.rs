...
pub trait StrategyParams {
    ...
    fn get_point_param_value(&self, name: Self::PointParam) -> PointSettingValue;
...
where
    ...
    RatioParam: Display,
{
    point_param_values: HashMap<ParamName, PointSettingValue>,
    ratio_param_values: HashMap<ParamName, PointSettingValue>,
        ...
        for param in reader.deserialize() {
                ...
                .is_alphabetic()
            {
                let numeric_setting_value =
                    (&param.value[..param.value.len() - 1]).parse::<PointSettingValue>()?;
                params
                    .ratio_param_values
                    .insert(param.name, numeric_setting_value);
            } else {
                let numeric_setting_value = param.value.parse::<PointSettingValue>()?;
                params
                    .point_param_values
                    .insert(param.name, numeric_setting_value);
            }
